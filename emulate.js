...
(async () => {
  const ColecoInstance = await ColecoAsync;
  console.log(ColecoInstance);
  const Coleco = ColecoInstance.exports;
  console.log(Coleco);
  const bios = Coleco.__newArray(
    Coleco.UINT8ARRAY_ID,
    Array.from(new Uint8Array(fs.readFileSync("./roms/bios.col")))
  );
  const cartridge = Coleco.__newArray(
    Coleco.UINT8ARRAY_ID,
    Array.from(new Uint8Array(fs.readFileSync("./roms/kevtris.col")))
  );

  console.log("Setup!");
  const ctx = document.getElementById("coleco").getContext("2d");
  ColecoInstance.refreshScreen = (videoData) => {
    const data = new ImageData(videoData, 272, 208);
  ...
  ColecoInstance.updateRegisters = (pc, a, f, bc, de, hl, ix, iy, i, r, sp) => {
    ...
    // memory.value = hexdump(
    //   Coleco.__getUint8Array(memoryPtr).slice(0x63f3, 0x6403).buffer
    // );
  };
